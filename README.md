...
**バックエンド専用（2026-07〜）。** API routes（Next.js Route Handler）+ Supabase migrations + MCP サーバーのみを含む。

アプリ本体はネイティブ iOS アプリ（別リポジトリ、TestFlight 配信中）。Web UI は廃止済みで、`/` へのアクセスは公式サイト [yuzu.style](https://yuzu.style)（別リポジトリ）へリダイレクトされる。Web 版はダウンロード導線・LP としてのみ存続し、アプリ機能そのものはこのリポジトリには無い。

作業規約は [CLAUDE.md](CLAUDE.md)、技術詳細は [ARCHITECTURE.md](ARCHITECTURE.md) を参照。
...
- **英語 = 状態・挑発**：`RECORDING` / `CARVING` / `CARVED` / `SILENCE`（状態 pill は句点なし）
- **日本語 = 事実・本文**：「長押し。話せ」「無音、話せ」

「癒し」「寄り添う」「頑張ろう」「育つ」「やさしく」は使わない。デザインシステムと UI コピー規約の source-of-truth は現在ネイティブ iOS リポジトリ側で管理している（DESIGN.md は廃止済み）。
...
| ドキュメント | 内容 |
|---|---|
| [PRD.md](PRD.md) | 製品要件・機能仕様・品質基準（Eval）・世界観。Web UI 前提の記述は歴史的文書として残る |
...
npm run dev                         # http://localhost:3000（API routes のみ。UI は無い）
```
