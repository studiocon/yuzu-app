...
- **本番未適用の migration が 5 本ある（2026-07-16 時点）**：
  ...
  - `20260716093000_create_record_rpc.sql` — 1 日録音上限の TOCTOU 原子化（#139）。advisory lock 付き `create_record` RPC で check+insert を直列化。**未適用の間は `app/api/records/route.ts` POST が `42883`（undefined_function）を検知して旧来の check-then-insert にフォールバックする**（＝原子性なしの従来挙動。二重 POST 競合は残る）。適用後に自動で原子化が有効になる
  - `20260716094000_start_report_job_rpc.sql` — レポート生成起動の TOCTOU 原子化（#143）。条件付き upsert の `start_report_job` RPC で起動権取得を原子化。**未適用の間は `lib/reports.ts` の `startReportJob` が `42883`（undefined_function）を検知して旧来の非原子 upsert にフォールバックする**（＝常に起動＝従来挙動。二重 POST で Claude 2 回呼びが残る）。前提として `report_jobs`（上記）が先に適用済みであること
