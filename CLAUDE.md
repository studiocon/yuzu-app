...
**OK 方向**：
- 命令形（話せ・出せ・黙るな）
- 状態英語（RECORDING. / CARVING. / CARVED. / SILENCE. / SAVING.）※完了スタンプは旧 RECORDED. を引退し CARVED. に統一。MARK/COPY のフラッシュ（MARKED / COPIED）はピリオドなし
- 句点（.）で終わる短文。詩的にしない・励まさない・優しくしない

**句点ルール**：
- **見出しは句点なし**。ページ名（`LOG` / `INSIGHT`）、セクション見出し、モーダルタイトル（`SIGN IN` / `MAIL` / `SENT` / `CONTACT` / `全部消す`）、シェアカード見出し（`DAY {N}` / `VOICE {N}`）は全て句点を付けない
- 句点が付くのは **状態 pill / バッジ**（RECORDING. / SAVING. 等）と **本文断定形短文**（説明・トースト・空状態・シェアキャプション等）に限る
