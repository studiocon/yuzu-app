...
> **本リポジトリはバックエンド専用になった（2026-07）。** UI 仕様の現行版はネイティブ iOS リポジトリ / Notion 側を参照。以下の Web UI 前提の記述は歴史的文書として残している。
...
- **EMOTION** — Claude API のセンチメントスコアを折れ線グラフで表示（[components/EmotionChart.tsx](components/EmotionChart.tsx)）
...
- **REPORTS** — 週次・月次レポートカードを一覧表示（[components/InsightView.tsx](components/InsightView.tsx)）
