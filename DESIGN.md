...
iOS 26 Liquid Glass を参考にした**フローティングピル型**ナビゲーション。フルブリードのバーではなく画面下に浮かぶ独立したカプセル。

```
位置: fixed・bottom: env(safe-area-inset-bottom) + 12px・水平中央
幅:   min(360px, 100vw - 32px)
高さ: 64px
形状: border-radius: 9999px（完全な楕円）
背景: rgba(250,250,245,0.72) + backdrop-filter: blur(40px) saturate(180%)
縁:   border: 1px solid rgba(255,255,255,0.6)
影:   drop-shadow(10px 30px 0.10) + inset highlight
```

**タブ構成（3タブ・順序固定）**

| 順 | タブ | アイコン | ラベル | 役割 | 情報の出どころ | デフォルト |
|---|------|---------|--------|------|----|----|
| 1 | INDEX | ListNumbers | "INDEX" | 自分が出した記録（#NNN・STATS・SENTIMENT・RECORDS） | ユーザー自身 | ✅ |
| 2 | TALK | Microphone | "TALK" | **中央のマイク FAB**。タップで RecordModal（飛び出しアニメ）を開く | ユーザーの行為 | — |
| 3 | REPORT | FileText | "REPORT" | AI 解釈の週次レポート | AI（他者視点）| |

- TALK は **タブ state を持たないアクション**（中央 `.mic-fab`）。押すと既存の `RecordModal`（fly アニメ）で録音 → CompleteView → 閉じる。
- アプリ起動時のデフォルト表示は **INDEX**。
- 性質分離：INDEX = ユーザー自身、REPORT = AI 解釈。旧「HOME」「ME」「PROFILE」は廃止。
...
**インタラクション**
- `:active` → `scale(0.94)`
- タブ切替はフェードなし即時（YUZU のトーンに合わせて派手にしない）
- `backdrop-filter` 非対応環境では `background: rgba(250,250,245,0.96)` にフォールバック

**オンボーディング中は非表示**（`data-hidden="true"` で opacity 0・translateY 8px・pointer-events none）
...
└──────────────────────────────┘
           ↑ タブコンテンツ
┌──[ INDEX ]──[ 🎤 TALK ]──[ REPORT ]──┐  ← フローティングピル（TALK は中央 FAB）
└──────────────────────────────────────┘
```

### INDEX 画面（自分が出した記録）

上から下へ「抽象 → 具体」で縦に積む。全セクション左揃え。**PROFILE・REPORT は含まない**。

```
┌─────────────────────────────┐
│ #042                         │  ← ① 通し番号（巨大・見出しを兼ねる / Unbounded 700 / 48px+）
│ DAY 14 | RECORDS 42 | STREAK 7│  ← ② STATS（横3列・中央揃えカード）
│ SENTIMENT                    │  ← ③ 感情チャート（折れ線）
│ ╱╲___╱╲                      │
...
- `#NNN` は最上部の背骨。INDEX の見出しを兼ねる（別途「INDEX」見出しは置かない）。**ユーザー identity は `#NNN` のみ**。名前・アイコンは持たない。
- `#NNN` 直下の細罫線（`--divider` の `border-top`）は STATS カードのトップ罫線を兼ねる。`#NNN` を **見出しとして区切る** 意図。装飾ではなく構造線。
- スタッツは **DAY / RECORDS / STREAK** の3つに削ぎ落とし。
- RECORDS フィルタは `ALL` / `PINNED` の1組のみ。narrow viewport（<400px）では `RECORDS` 見出しが大きいためフィルタが下段に **折り返す**ことを許容する（`flex-wrap: wrap`）。意図的。
- 設定アイコンはヘッダー右上に据え置く。
...
```
┌─────────────────────────────┐
│ REPORT                       │  ← Unbounded 700 / 48px / 左揃え
│ ┌──[ WEEK #2 ]──┐ ┌──[ #1 ]┐│  ← 週次/月次レポートカード（既存）
│ └────────────────┘ └────────┘│
└─────────────────────────────┘
```

- 既存の [components/ReportsSection.tsx](components/ReportsSection.tsx) を本体として描画。新規ロジックは持たない（INDEX からの移設のみ）。
- 詳細遷移は `/reports`（"MORE →" 既存）。
...
|---|---|---|---|
| TALK | トーク | はなす画面。アプリのデフォルト面。開いたら即マイク。 | タブラベル英語=Unbounded。日本語UIでは「話す」 |
| INDEX | インデックス | **ユーザー自身が出した**記録の集約面（#NNN・STATS・SENTIMENT・RECORDS）。 | REPORT・PROFILE は含まない |
| REPORT | レポート | **AI** がユーザーの声を解釈して返す週次レポート。独立タブ。 | INDEX とは情報の出どころが違うため分離 |
| `#NNN` | ナンバー | ユーザーの **identity 兼通し番号**。名前・アイコンの代替。 | 「名前は無い。お前は #020 だ」。ゼロ埋め3桁。INDEX 見出しを兼ねる |
...
| TALK/INDEX/REPORT 3タブ | — | 最終タブ構成。性質分離の結果（バランス目的の追加ではない）。 | ホーム・ME・PROFILE は廃止済み |
