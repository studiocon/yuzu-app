...
| 語幹 | -ING.（進行中） | -ED.（完了） | 意味 |
|---|---|---|---|
| **RECORD** | RECORDING. | RECORDED. | 録音 / 記録（DB 保存まで完了） |
| **CARVE** | CARVING. | CARVED. | 声→テキスト変換 / 刻印（保存は別ステップ） |
...
**メタルール**：
- `CARVE` = 声を**刻む**（変換そのもの、STT という行為）
- `RECORD` = 声を**残す**（DB に保存して記録として成立させる）
- オンボーディング preview の `CARVED.` は「刻まれた」が「まだ残されていない」状態 — そのため次のステップでログインして `RECORDED.` （記録された）に昇格する
