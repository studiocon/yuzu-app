...
| タブ | アイコン | ラベル | 役割 |
|------|---------|--------|------|
| HOME | House | "HOME" | RECORDS タイムライン |
| TALK | Microphone | "TALK" | 録音モーダルを開く（タブ state は変わらない） |
| ME | User | "ME" | 統計・SENTIMENT・REPORT・設定リンク |
...
└──────────────────────────────┘
           ↑ タブコンテンツ
┌──[ HOME ]──[ TALK ]──[ ME ]──┐  ← フローティングピル
└──────────────────────────────┘
```

### マイページ画面（ME タブ）

- ヘッダー：アバター絵文字 ＋ ニックネーム（LINE Seed JP 700）
- スタッツ：左揃え・罫線で区切り・数値は巨大・ラベルは小さく
- 感情分析チャート：transparent 背景・墨とゆずオレンジの2線
- REPORT セクション：週次レポートカード
- ACCOUNT セクション：設定リンク（`/settings` へ遷移）
- RECORDS リストは ME から削除 → HOME タブに分離
