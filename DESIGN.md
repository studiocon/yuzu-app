...
| INSIGHT セクションラベル | EMOTION / SIGNAL / WORDS / PATTERN / REPORTS（句点なし・Unbounded） |
...
| PATTERN 順位 | `#N`（左、Unbounded、--ink-muted） |
| PATTERN シェア率 | `NN%`（右、Unbounded、--ink） |
...
投稿カードの `#NNN` をタップすると開く全画面モーダル（`--ink` 背景・白文字）。
...
│  ┌──────────┐ ┌──────────┐           │
│  │ LENGTH   │ │ DAY      │           │ ← 事実 STATS（主役）
│  │ 1:24     │ │ 14       │           │
│  └──────────┘ └──────────┘           │
│  2026.05.29 (木) 23:11               │ ← スタンプ（曜日入り）
│  今日はずっと機嫌が良かった。…       │ ← 本文（二次情報・ミュート）
└──────────────────────────────────────┘
```

- **役割は「その1件の RECORD を事実として突きつける」**（THE RECORD 思想・ミラー）。本文を大きく見せる場ではない。
- `#NNN` は identity の核として残すが、投稿カードからの遷移で過剰にならないようサイズを抑える（`clamp(64px, 18vw, 120px)`）。
...
- 本文は再読用の二次情報に格下げ（`--text-base` / `rgba(255,255,255,.6)`）。
...
**使用箇所**
- LOG > RECORDS の `ALL` / `PINNED`（[components/IndexView.tsx](components/IndexView.tsx)）
- INSIGHT > EMOTION の `MONTH` / `ALL`（[components/LongSentimentChart.tsx](components/LongSentimentChart.tsx)）
...
- EMOTION: [components/LongSentimentChart.tsx](components/LongSentimentChart.tsx)
...
| INSIGHT | インサイト | **AI** がユーザーの声を解釈して返す面（EMOTION・REPORTS）。 | LOG とは情報の出どころが違うため分離。旧称「READ」「REPORT」 |
...
| EMOTION | エモーション | 声の感情推移を示す折れ線チャート。 | 「気づき」「自分を知ろう」と書かない。旧称「SENTIMENT」 |
