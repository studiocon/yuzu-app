...
### INDEX 詳細 (`.index-detail-modal` in [components/IndexDetailModal.tsx](components/IndexDetailModal.tsx)) — v2.3
...
- **見出し帯**（`.index-detail-band`）が `#NNN`・声紋ヒーロー・スタンプをまとめ、その RECORD の感情スコア由来のグラデーション（低 alpha → 透明）を背後に敷く。score 未解析なら無色（graceful）。`box-shadow` は使わず**背景グラデのみ**で帯を作る。
...
- **WORDS 自動ハイライト**（`.record-mark`）：その RECORD の内容語（[lib/wordAnalysis.ts](lib/wordAnalysis.ts) の TinySegmenter 抽出をクライアントで再利用。漢字・カタカナを含む 2 文字以上のみ＝機能語/単漢字ノイズを除外）を本文中で `--yuzu-yellow` ＋細い下線で強調する。**文言は一切変えない**（声をそのまま強調するだけ。「気づき」等の説明は足さない）。AI は使わない（コスト 0・オフライン/mock 動作可）。
