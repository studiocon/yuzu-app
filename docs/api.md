...
| 問い合わせ（`/api/inquiries` POST） | `INQUIRY_RATE_MAX = 5` 件 / `INQUIRY_RATE_WINDOW_MS`（1時間） | **優先順位**: `user_id` → `email` → **IP**（#129 で追加。両方欠く匿名 POST が素通りしていた問題への対応） | `429 { error: "rate_limited", retryAfterMs }` |
...
`ReportMeta` 形状: `{ periodKey, kind: "week"|"month", rangeStart, rangeEnd, label, generated, headline?, topics?, postCount, payload?: ReportPayload, generatedAt?, model? }`
...
- レート制限: 5件/時。判定キーは `user_id` → `email` → **IP**（#129、両方欠く匿名 POST への防波堤）
