...
2. **UI コピー整合性**
   ...
   - 例：`RECORDED.` / `RECORDING.` / `DECODING.` / `SILENCE.` / `閉じる` / `失敗、話せ` / `無音、話せ` / `短い、話せ` / `話せ` / `出せ` / `整えるな` / `長押し。話せ`
...
4. 実装の主要文言（`grep -rn "RECORDING\\.\\|DECODING\\.\\|RECORDED\\.\\|SILENCE\\.\\|閉じる" app components`）を取得し、表と突合
