...
2. **UI コピー整合性**
   ...
   - 例：`RECORDING` / `CARVING` / `CARVED` / `SILENCE` / `閉じる` / `失敗、話せ` / `無音、話せ` / `短い、話せ` / `話せ` / `出せ` / `整えるな`（**状態 pill は UI ＝ 句点なし**。旧 `DECODING.`/`DECODED.` は `CARVING`/`CARVED` に改称済。完了スタンプの旧 `RECORDED.` は引退し `CARVED` に統一。句点が付くのは説明文のみ）
...
4. 実装の主要文言（`grep -rn "RECORDING\\|CARVING\\|CARVED\\|SILENCE\\|閉じる" app components`）を取得し、表と突合（状態 pill に句点が残っていれば外す。旧 `RECORDED.` が残っていれば `CARVED` に直す）
